#!/usr/bin/env python3
# filepath: d:\Workspace\flutter\social_app\server\backend\run_azure_server.py
"""
Script để khởi động server ASGI với WebSocket trên máy ảo Azure.
...
def start_redis_daemon():
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            logger.error(f"Lỗi khi khởi động Redis: {result.stderr}")
            return False
    except Exception as e:
